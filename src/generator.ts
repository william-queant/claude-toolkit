...
import { copyDir, writeFileEnsureDir, readJson, exists } from './utils.js';
import type { ClaudeToolkitConfig, StackPack, ResolvedConfig } from './types.js';
...
async function resolveConfig(config: ClaudeToolkitConfig): Promise<ResolvedConfig> {
  ...
  const installCmd = config.hooks?.installCommand
...
export async function generate(projectDir: string, config: ClaudeToolkitConfig): Promise<void> {
  ...
  await writeFileEnsureDir(join(claudeDir, '.gitignore'), [
  ...
  console.log(`Generated .claude/ with ${resolved.stacks.length} stack(s) and ${resolved.skills.length} core skills`);
}
...
async function generateSkillRules(claudeDir: string, resolved: ResolvedConfig): Promise<void> {
  ...
  for (const stackName of resolved.config.stacks) {
    ...
    if (exists(stackJsonPath)) {
      const pack = await readJson<StackPack & { skillRules?: Record<string, unknown> }>(stackJsonPath);
      if (pack.skillRules) {
  ...
  const skillRules = {
    ...
    skills: {
      ...(baseRules as Record<string, unknown>).skills,
      ...rules,
    },
  };
